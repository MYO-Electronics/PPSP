...
% output at current iteration
    y = w' * X;
%     qrsFcfica=FecgQRSfDet(y',fs,'02',qrsM,0,0,0,0,qrsAf);
%     figure
%     plot(y)
%     hold on
%     plot(qrsAf/1000*fs,1,'r+');
%     plot(qrsFcfica*fs,0.5,'k+')
%     evaluation(qrsFcfica*fs/inter,qrsAf,1);
%     evaluation(qrsFcfica*fs/inter+15,qrsAf,1);
...
tic
   ...
   toc
    disp(['gdg21 Running Time: ',num2str(toc)]);
    ...
    qrsFcfica=FecgQRSfDetCfica(y',fs,num2str(user),qrsM,0,0,0,0,qrsAf);
%     figure
%     plot(y)
%     hold on
%     plot(qrsAf/1000*fs,1,'r+');
%     plot(qrsFcfica*fs,1,'k+')
    ...
    if spike{4} == 100
        fprintf('100% F1-score after %d iteration\n',loop);
        flag = 0;
    end
